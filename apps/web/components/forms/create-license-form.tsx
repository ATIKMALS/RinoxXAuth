    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      {generatedKey ? (
        ...
        <>
          {/* App Select */}
          <div>
            <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1.5">Select Application <span className="text-red-400">*</span></label>
            {fetchError ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3">
                <div className="flex items-center justify-between">
                  <p className="text-sm text-amber-400 flex items-center gap-2"><AlertCircle className="h-4 w-4" />{fetchError}</p>
                  <button type="button" onClick={fetchApps} className="p-2 rounded-lg bg-amber-500/10 text-amber-400"><RefreshCw className="h-4 w-4" /></button>
                </div>
              </div>
            ) : (
            ...
            {errors.app_id && <p className="text-xs text-red-400 mt-1"><AlertCircle className="h-3 w-3 inline" />{errors.app_id}</p>}
          </div>
          ...
          <label className="flex items-center gap-3 p-3 rounded-xl border border-zinc-700/50 bg-zinc-800/30 cursor-pointer hover:border-indigo-500/30 transition-all">
            <input type="checkbox" checked={formData.use_custom_key} onChange={(e) => setFormData({ ...formData, use_custom_key: e.target.checked })}
          ...
          {formData.use_custom_key && (
            <div className="relative">
              ...
              <input type="text" value={formData.custom_key} onChange={(e) => setFormData({ ...formData, custom_key: e.target.value })}
              ...
              <button type="button" onClick={generateKey} className="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1 rounded-lg bg-indigo-500/10 text-indigo-400 text-xs">Generate</button>
            </div>
          )}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="number" min="0" max="3650" value={formData.expiry_days} onChange={(e) => setFormData({ ...formData, expiry_days: parseInt(e.target.value) || 0 })}
          ...
          {/* Note */}
          <div>
            <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1.5">Note <span className="text-zinc-600">(Optional)</span></label>
            <div className="relative">
              ...
              <textarea value={formData.note} onChange={(e) => setFormData({ ...formData, note: e.target.value })}
                placeholder="e.g., For Client ABC" rows={2}
          ...
          <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-3">
            <p className="text-xs text-indigo-300">Generate key → Give to user → User registers with username & password</p>
          </div>

          {errors.submit && <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-3"><p className="text-xs text-red-300">{errors.submit}</p></div>}

          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 rounded-xl border border-zinc-700/50 bg-zinc-800/50 px-4 py-2.5 text-sm text-zinc-300 hover:bg-zinc-700/50 transition-all">Cancel</button>
            <Button type="submit" disabled={isLoading} className="flex-1 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-500 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-indigo-500/25 transition-all disabled:opacity-50">
              {isLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null} Generate Key
            </Button>
          </div>
        </>
      )}
