...
import { ApiResponse, UserRecord } from "@/lib/types";
        ...
        <div className="p-4 border-b border-zinc-800/50">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input placeholder="Search users..." className="w-full rounded-xl border border-zinc-700/50 bg-zinc-800/50 pl-10 pr-4 py-2.5 text-sm text-white placeholder-zinc-500 focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all" />
            </div>
              ...
              {users.length === 0 ? (
                  ...
                  <td colSpan={6} className="px-4 py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm text-zinc-500 max-w-md">
                        Create your first user to get started with user management and license validation.
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                users.map((user: any, index: number) => (
                  <tr key={user.id || index} className="group border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-all duration-300">
                    ...
                    <td className="px-4 py-4">
                      <span className="text-sm text-zinc-400 font-mono">{user.hwid ? "1/1" : "0/1"}</span>
                    </td>
                    <td className="px-4 py-4">
                      <span className="text-sm text-zinc-400">{user.expires_at?.slice(0, 10) || "N/A"}</span>
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-1 justify-end opacity-0 group-hover:opacity-100 transition-all duration-300">
                        ...
                        <button className="p-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-indigo-400 transition-all duration-300 hover:scale-110" title="Edit User">
                        ...
                        <form action={async (formData: FormData) => {
                          ...
                          await fetch(`${env.BACKEND_BASE_URL}/api/admin/users/${uid}`, {
                            method: "PATCH",
                          ...
                          <button type="submit" className={`p-2 rounded-lg bg-zinc-800 transition-all duration-300 hover:scale-110 ${
                            user.status === "banned" 
                              ? "hover:bg-emerald-500/20 text-zinc-400 hover:text-emerald-400" 
        ...
        {users.length > 0 && (
          <div className="p-4 border-t border-zinc-800/50 flex items-center justify-between">
            <p className="text-sm text-zinc-500">
              Showing <span className="text-white font-medium">{users.length}</span> user{users.length !== 1 ? "s" : ""}
            </p>
          </div>
        )}
      </Card>
